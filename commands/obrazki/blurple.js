...
module.exports.run = async (client, message, args) => {
    util.searchUser(message, args[0]).then(user => {
        snek.get(`http://api.badosz.com/blurple?url=${user.avatarURL}`).set({ Authorization: config.tokens.badosz}).then(response => {
            var embed = new Discord.RichEmbed();
            embed.attachFile({attachment: response.body, name: 'blurple.png'});
...
module.exports.help = {
    name:"blurple",
    name2:"blurple [mention/nazwa użytkownika]",
